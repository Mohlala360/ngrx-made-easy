...
export class FoodEffects {
  ...
  @Effect() run$ = this.actions$.pipe(
    ...
    switchMap((payload) =>
      this.apiService
        .run(payload ? payload["name"] : null)
        .pipe(map((result) => new CreateFoodSuccess(<Food>result)))
    )
  );

  @Effect() update$ = this.actions$.pipe(
    // Listen to CREATE_FOOD action
    ofType(FoodActionTypes.UPDATE_FOOD),
    // Fetch payload
    map((action: FoodAction) => action.payload),
    // Call API or any other desired stuff
    switchMap((payload) =>
      this.apiService
        .update(payload ? payload["name"] : null)
        .pipe(map((result) => new UpdateFoodSuccess(<Food>result)))
    )
  );
}
